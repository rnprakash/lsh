...
\begin{algorithm}
    ...
    \Input{$t = (\ldots, t_i, \ldots) \in S$, real-time time series;\\
        ...
        $M \in \N$;\\
        $d \colon H \times H \rightarrow \R^+$, distance metric;\\
    ...
    \For{$k \in K$}{
        \If{$\mathtt{len}(t) \bmod k = 0$}{
            Let $r$ be the corresponding window size for $k$.\\
            Let $t_k$ = $t[-k:]$\\
            $P = \{ \pi_{h_a}(\phi(t_k)\}$ \tcc*[f]{project $\phi(t_k)$ onto $a$ random hyperplanes $h_a$}\\
            Define $v_P(r) = \min\limits_{p \in P}[\frac{d(\bm{0}, p)}{r} \bmod M]$\\
            $D = \{ v_P(r) \mid r \in R \}$ \tcc*[f]{min distance from origin to projected point in units $r$}\\
            Define $R_i = \{ r_j \in R \mid j \ge i \}$\\
            \If{$\exists~i$ such that $\forall r_j \in R_i, |\{A_r(v_P(r_j)) = 0\}| \ge |\frac{R_i}{2}|$}{
                \Raise{anomaly}
            }
        }
    }
...
\begin{proof}
    \begin{align*}
        \norm{x + y}_H = \sqrt{\sum\limits_i{(x_i + y_i)^2} + \sum\limits_i{(D^1 (x_i + y_i))^2}} \\
        \le \sqrt{\sum\limits_i{x_i^2} + \sum\limits_i{y_i^2} + \sum\limits_i{(D^1 x_i)^2} + \sum\limits_i{(D^1 y_i)^2}}\\
        \le \sqrt{\sum\limits_i{x_i^2} + \sum\limits_i{(D^1 x_i)^2}} + \sqrt{\sum\limits_i{y_i^2} + \sum\limits_i{(D^1 y_i)^2}}\\
        = \norm{x}_H + \norm{y}_H \numberthis
    \end{align*}
...
\begin{proof}
    ...
    $\kappa(f, g) = \innerproduct{\phi(f)}{\phi(g)}_H$ is an inner product on $H$, and therefore defines a unique positive-definite kernel (by the positive-definite property of the inner product).
\end{proof}
