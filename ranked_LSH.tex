...
\begin{algorithm}[H]
    ...
    \For{$k \in K$}{
        \If{$\mathtt{len}(t) \bmod k = 0$}{
            ...
            $D = \{ v_P(r) \mid r \in R \}$ \tcc*[f]{min distance from origin to projected point for each unit $r$}\\
            Define $R_i = \{ r_j \in R \mid j \ge i \}$\\
            \If{$\exists~R_i$ such that $|\{A(r,v_P(r_j)) = 0 \mid \forall r_j \in R_i\}| \ge \frac{\cardinality{R_i}}{2}$}{
                \Raise{anomaly}
            }
        }
    }
