...
\begin{definition}\label{defn:hilbert_space}
    \textbf{Hilbert space}:
    A vector space $H$ over a field $F$ with an inner product $\innerproduct{\cdot}{\cdot}_H: H \times H \rightarrow F$ that also defines a complete\footnote{A space $X$ is complete if every Cauchy sequence  converges in $X$. A Cauchy sequence is a sequence $\{x_n\}_{n \in \mathbb{N}}, x_n \in X$ with $\lim\limits_{(m,n)\rightarrow\infty} |x_m - x_n| = 0$.} metric space is called a Hilbert space.
\end{definition}
...
\begin{definition}
    ...
    A kernel function $\kappa(\cdot, \cdot)$ is a similarity measure on $X$ if $\kappa(x,x) = 1$ for any $x\in X$ and $\kappa(x,y)$ decreases as $d(x,y)$ increases for some distance measure $d$.
\end{definition}
...
Consider a set of $n$ samples $D = \{x_1,\ldots,x_n\} \subset \mathbb{R}^d$ with similarity measure $\kappa \colon D \times D \rightarrow \mathbb{R}$ defined by $\kappa(x,y) = \innerproduct{\phi(x)}{\phi(y)}_H$, with $H$ being a RKHS.
...
\begin{definition}\label{defn:kernel_lsh}
    \textbf{Kernelized LSH}:
    %Let $\kappa_r(\cdot, \cdot) = \kappa()$ 
    $\mathcal{H}_R = \{ \mathcal{H}_r \}_{r \in R}$ is a $(R, p, q)$-sensitive ranked, kernelized LSH family if, for any $x,y \in D$:
    \begin{enumerate}[(i)]
        \item If $\tilde{\kappa}(x,y) \le r, \Pr[h_r(x) = h_r(y)] > p$
        \item If $\tilde{\kappa}(x,y) \ge r, \Pr[h_r(x) = h_r(y)] < q$
        \item Property $\mathscr{A}$ holds for any two LSH families $\mathcal{H}_{r_1}, \mathcal{H}_{r_2} \in \mathcal{H}_R$
    \end{enumerate}
...
\begin{definition}\label{defn:neighborhood_rank}
    Let $X$ be an input space and $\mathcal{H}_R$ an RKLSH, with $R = \{r_1, r_2, \ldots\}$.
    Two data samples $x,y \in X$ have \textbf{rank} $n$ if $n = \inf\limits_n\{ r_n \in R | h_{r_n}(x) = h_{r_n}(y) \}$.
\end{definition}

By applying this tiered nearest neighbor calculation, we are able to \textit{rank} the similarity of two elements by examining the threshold at which their hashes collide.

\iffalse
% This doesn't work
% TODO: fix
Define an equivalence relation $\sim_r$ on $S$ where $x \sim_r y \Leftrightarrow \tilde{\kappa}(x,y) < r$.
Then, $\pi_r$ is a projection mapping of $\sim_r$ such that $\pi_r \colon X \rightarrow X/\sim_r$, where $\pi_r(x) = \pi_r(y)$ iff $\tilde{\kappa}(x,y) < r$.
