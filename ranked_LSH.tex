...
Hashing has long been used as a method of easing the curse of dimensionality when performing costly pairwise computations on a large set of high-dimensional data~\cite{Indyk98-ANN,Gionis99-SSH,Datar04-LSH}.
...
\begin{enumerate}[*]
    \item\label{itm:hash_def3}
        A family $\mathcal{H}_r$ obeys the triangle inequality w.r.t. $d(\cdot,\cdot)$ such that it strictly obeys Definition~\ref{def:hash_family} for a set of distances $R$.
        In other words, $\mathcal{H}_r$ is a $(r_i,r_i,p_1,p_2)$-sensitive hash family for every $r_i \in R$.
\end{enumerate}
...
\begin{proposition}\label{prop:ranked_lsh}
    There exists a hash family $\mathcal{H}_r$ such that the following three hold:
    \begin{enumerate}[(i)]
        \item\label{itm:first} $h \in \mathcal{H}_r$ \textit{collides} with high probability if $x$ and $y$ are ``close'' ($d(x,y) < r \Rightarrow \Pr[h(x) = h(y)] \ge p_1$)
        \item\label{itm:second} $h \in \mathcal{H}_r$ has few \textit{false collisions}: If $d(x,y) > r$, $\Pr[h(x) = h(y)] < p_2$ for some fixed, small $p_2$.
        \item\label{itm:third} $\mathcal{H} = {\{\mathcal{H}_r\}}_{r\in R}$ obeys (\ref{itm:first}) and (\ref{itm:second}) for any finite $R$.
    \end{enumerate}
\end{proposition}
